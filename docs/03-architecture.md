# Levelly Architecture

This document explains how the different parts of Levelly connect together and work as a system.

## Table of Contents
- [System Overview](#system-overview)
- [Technology Stack](#technology-stack)
- [Component Architecture](#component-architecture)
- [Data Flow Diagrams](#data-flow-diagrams)
- [Folder Structure](#folder-structure)

---

## System Overview

Think of Levelly like a restaurant:

| Restaurant | Levelly |
|------------|---------|
| The dining area (what customers see) | **Frontend** - The website users interact with |
| The kitchen (where food is prepared) | **Backend** - Processes data and logic |
| The recipe book & ingredient storage | **Database** - Stores all information |
| A special chef who grades taste tests | **AI (OpenAI)** - Grades open-ended answers |

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                           LEVELLY SYSTEM OVERVIEW                           │
└─────────────────────────────────────────────────────────────────────────────┘

                              ┌─────────────────┐
                              │     USERS       │
                              │                 │
                              │  👩‍🏫 Teachers    │
                              │  👨‍🎓 Students    │
                              └────────┬────────┘
                                       │
                                       │ Browse website
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                              FRONTEND (Next.js)                              │
│                                                                              │
│   What users see and interact with:                                          │
│   • Login/Signup pages                                                       │
│   • Teacher dashboard                                                        │
│   • Quiz creation forms                                                      │
│   • Quiz taking interface                                                    │
│   • Results page                                                             │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
                                       │
                                       │ API calls
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                              BACKEND (Next.js API)                           │
│                                                                              │
│   The "brain" that processes everything:                                     │
│   • Handles user authentication                                              │
│   • Creates/updates quizzes                                                  │
│   • Grades MCQ questions instantly                                           │
│   • Sends open questions to AI for grading                                   │
│   • Calculates levels                                                        │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
                          │                        │
                          │                        │
                          ▼                        ▼
┌──────────────────────────────────┐    ┌──────────────────────────────────┐
│         DATABASE (Supabase)       │    │           AI (OpenAI)            │
│                                   │    │                                  │
│   Stores everything:              │    │   Grades open-ended answers:     │
│   • Teacher accounts              │    │   • Reads student response       │
│   • Quizzes & questions           │    │   • Understands meaning          │
│   • Student answers               │    │   • Assigns score & feedback     │
│   • Scores & levels               │    │                                  │
│                                   │    │                                  │
└──────────────────────────────────┘    └──────────────────────────────────┘
```

---

## Technology Stack

### What Each Technology Does

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                           TECHNOLOGY STACK                                   │
└─────────────────────────────────────────────────────────────────────────────┘

   FRONTEND (What you see)
   ════════════════════════
   ┌─────────────────────────────────────────────────────────────────────────┐
   │                                                                         │
   │   Next.js ─────► The framework that builds the website                  │
   │      │                                                                  │
   │      └──► React ─────► Makes pages interactive (buttons, forms, etc.)   │
   │              │                                                          │
   │              └──► TypeScript ─────► The programming language used       │
   │                        │                                                │
   │                        └──► Tailwind CSS ─────► Makes things look pretty│
   │                                                                         │
   └─────────────────────────────────────────────────────────────────────────┘

   BACKEND (Behind the scenes)
   ═══════════════════════════
   ┌─────────────────────────────────────────────────────────────────────────┐
   │                                                                         │
   │   Next.js API Routes ─────► Handles requests from the frontend          │
   │          │                                                              │
   │          ├──► Authentication ─────► Login/logout/signup                 │
   │          │                                                              │
   │          ├──► Quiz Logic ─────► Creating, updating, submitting quizzes  │
   │          │                                                              │
   │          └──► Grading ─────► MCQ (instant) + Open (via AI)              │
   │                                                                         │
   └─────────────────────────────────────────────────────────────────────────┘

   DATA STORAGE
   ════════════
   ┌─────────────────────────────────────────────────────────────────────────┐
   │                                                                         │
   │   Supabase ─────► Cloud database service                                │
   │       │                                                                 │
   │       ├──► PostgreSQL ─────► The actual database (stores tables)        │
   │       │                                                                 │
   │       └──► Auth ─────► Handles teacher login securely                   │
   │                                                                         │
   └─────────────────────────────────────────────────────────────────────────┘

   ARTIFICIAL INTELLIGENCE
   ═══════════════════════
   ┌─────────────────────────────────────────────────────────────────────────┐
   │                                                                         │
   │   OpenAI API ─────► The AI service that grades open answers             │
   │       │                                                                 │
   │       └──► GPT Model ─────► Understands text and provides scores        │
   │                                                                         │
   └─────────────────────────────────────────────────────────────────────────┘
```

### Simple Analogies

| Technology | What It's Like |
|------------|----------------|
| **Next.js** | The building that houses everything |
| **React** | The interior design - makes things look good and work well |
| **TypeScript** | The language everyone speaks inside |
| **Tailwind CSS** | The paint and decorations |
| **Supabase** | The filing cabinets storing all documents |
| **OpenAI** | A smart assistant who reads and grades essays |

---

## Component Architecture

### Page Structure

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         WEBSITE PAGE STRUCTURE                               │
└─────────────────────────────────────────────────────────────────────────────┘

                              levelly.com
                                  │
          ┌───────────────────────┼───────────────────────┐
          │                       │                       │
          ▼                       ▼                       ▼
    PUBLIC PAGES            TEACHER PAGES            QUIZ PAGES
    ════════════            ═════════════            ══════════
          │                       │                       │
    ┌─────┴─────┐           ┌─────┴─────┐           ┌─────┴─────┐
    │           │           │           │           │           │
    ▼           ▼           ▼           ▼           ▼           ▼
 ┌──────┐  ┌───────┐  ┌─────────┐  ┌────────┐  ┌──────┐  ┌─────────┐
 │ Home │  │ Login │  │Dashboard│  │ Quiz   │  │ Take │  │ Results │
 │  /   │  │/login │  │/teacher │  │ Edit   │  │ Quiz │  │/results │
 │      │  │       │  │         │  │        │  │/quiz │  │  /[id]  │
 │      │  │/signup│  │         │  │/assign-│  │/[tok]│  │         │
 │      │  │       │  │         │  │ments/  │  │      │  │         │
 │      │  │       │  │         │  │ [id]   │  │      │  │         │
 └──────┘  └───────┘  └─────────┘  └────────┘  └──────┘  └─────────┘

    │           │           │           │           │           │
    │           │           │           │           │           │
    ▼           ▼           ▼           ▼           ▼           ▼
  Anyone     Anyone      Teachers    Teachers    Students    Students
  can see   can access   only        only       (via link)  (after
                         (login                              submit)
                         required)
```

### How Pages Connect to Data

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                       PAGE → DATA CONNECTIONS                                │
└─────────────────────────────────────────────────────────────────────────────┘

   ┌─────────────────┐          ┌─────────────────┐          ┌─────────────────┐
   │   QUIZ PAGE     │          │   API ROUTES    │          │    DATABASE     │
   │  (What student  │   ───►   │  (The middle-   │   ───►   │  (Where data    │
   │   sees)         │          │   man)          │          │   lives)        │
   └─────────────────┘          └─────────────────┘          └─────────────────┘

   Example Flow - Student takes quiz:
   ══════════════════════════════════

   1. Student opens quiz link
           │
           ▼
   ┌─────────────────────────────────────────────────────────────────────────┐
   │  Browser shows: /quiz/abc123                                            │
   │                                                                         │
   │  Page needs quiz data, so it calls:                                     │
   │  GET /api/quiz/abc123                                                   │
   └─────────────────────────────────────────────────────────────────────────┘
           │
           ▼
   ┌─────────────────────────────────────────────────────────────────────────┐
   │  API Route receives request                                             │
   │                                                                         │
   │  1. Looks up share link "abc123" in database                            │
   │  2. Gets assignment (quiz) details                                      │
   │  3. Gets all questions for this quiz                                    │
   │  4. Returns data to the page                                            │
   └─────────────────────────────────────────────────────────────────────────┘
           │
           ▼
   ┌─────────────────────────────────────────────────────────────────────────┐
   │  Page displays the quiz with:                                           │
   │  • Title & description                                                  │
   │  • All questions                                                        │
   │  • Answer inputs                                                        │
   └─────────────────────────────────────────────────────────────────────────┘
```

---

## Data Flow Diagrams

### Quiz Submission Flow (The Most Complex Part)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    QUIZ SUBMISSION - COMPLETE FLOW                           │
└─────────────────────────────────────────────────────────────────────────────┘

   STUDENT                    SERVER                    DATABASE              AI
   ═══════                    ══════                    ════════              ══

      │                          │                          │                  │
      │  1. Submit answers       │                          │                  │
      │ ────────────────────────►│                          │                  │
      │                          │                          │                  │
      │                          │  2. Create attempt       │                  │
      │                          │ ────────────────────────►│                  │
      │                          │         (new record)     │                  │
      │                          │                          │                  │
      │                          │  3. Save all answers     │                  │
      │                          │ ────────────────────────►│                  │
      │                          │                          │                  │
      │                          │                          │                  │
      │                          │  4. Grade MCQs           │                  │
      │                          │  ┌────────────────────┐  │                  │
      │                          │  │ For each MCQ:      │  │                  │
      │                          │  │ • Compare answer   │  │                  │
      │                          │  │ • Award points     │  │                  │
      │                          │  └────────────────────┘  │                  │
      │                          │         (instant!)       │                  │
      │                          │                          │                  │
      │  5. Return MCQ score     │                          │                  │
      │ ◄────────────────────────│                          │                  │
      │  (Student can see        │                          │                  │
      │   provisional results)   │                          │                  │
      │                          │                          │                  │
      │                          │  6. Background: Grade open questions        │
      │                          │  ┌───────────────────────────────────────┐  │
      │                          │  │                                       │  │
      │                          │  │  For each open question:              │  │
      │                          │  │                                       │──►│
      │                          │  │  • Send to AI                         │  │
      │                          │  │                      7. AI analyzes   │  │
      │                          │  │                         and grades    │  │
      │                          │  │                                       │◄─┤
      │                          │  │  • Receive score + feedback           │  │
      │                          │  │                                       │  │
      │                          │  │  • Save to database ─────────────────►│  │
      │                          │  │                                       │  │
      │                          │  └───────────────────────────────────────┘  │
      │                          │                          │                  │
      │                          │  8. Calculate final level│                  │
      │                          │ ────────────────────────►│                  │
      │                          │     (update attempt)     │                  │
      │                          │                          │                  │
      │  9. Poll for updates     │                          │                  │
      │ ────────────────────────►│                          │                  │
      │                          │                          │                  │
      │  10. Return final score  │                          │                  │
      │ ◄────────────────────────│                          │                  │
      │                          │                          │                  │
   ┌──┴──┐                       │                          │                  │
   │Shows│                       │                          │                  │
   │final│                       │                          │                  │
   │level│                       │                          │                  │
   └─────┘                       │                          │                  │
```

### Authentication Flow

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         AUTHENTICATION FLOW                                  │
└─────────────────────────────────────────────────────────────────────────────┘

   TEACHER SIGN UP
   ═══════════════

   ┌──────────┐     ┌──────────┐     ┌──────────┐     ┌──────────┐
   │  Enter   │     │ Supabase │     │  Create  │     │ Redirect │
   │  email,  │────►│  creates │────►│ teacher  │────►│   to     │
   │ password │     │  account │     │  profile │     │dashboard │
   └──────────┘     └──────────┘     └──────────┘     └──────────┘


   TEACHER LOGIN
   ═════════════

   ┌──────────┐     ┌──────────┐     ┌──────────┐     ┌──────────┐
   │  Enter   │     │ Supabase │     │  Create  │     │ Redirect │
   │  email,  │────►│ verifies │────►│ session  │────►│   to     │
   │ password │     │  creds   │     │  cookie  │     │dashboard │
   └──────────┘     └──────────┘     └──────────┘     └──────────┘


   ROUTE PROTECTION (Middleware)
   ═════════════════════════════

   Someone visits /teacher/*
            │
            ▼
   ┌────────────────────┐
   │ Check: Is user     │
   │ logged in?         │
   └─────────┬──────────┘
             │
       ┌─────┴─────┐
       │           │
      YES          NO
       │           │
       ▼           ▼
   ┌────────┐  ┌────────┐
   │ Allow  │  │Redirect│
   │ access │  │to login│
   └────────┘  └────────┘
```

---

## Folder Structure

Here's what each folder contains and why:

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                          PROJECT FOLDER STRUCTURE                            │
└─────────────────────────────────────────────────────────────────────────────┘

levelly/
│
├── 📁 src/                          ← All source code lives here
│   │
│   ├── 📁 app/                      ← Pages and routes
│   │   │
│   │   ├── 📁 (root)/               ← Home page
│   │   │   └── page.tsx             ← What you see at levelly.com/
│   │   │
│   │   ├── 📁 api/                  ← Backend API endpoints
│   │   │   ├── 📁 quiz/             ← Quiz-related APIs
│   │   │   │   ├── [token]/         ← Get quiz by share link
│   │   │   │   └── submit/          ← Submit quiz answers
│   │   │   ├── 📁 attempts/         ← Student attempt APIs
│   │   │   └── 📁 assignments/      ← Teacher quiz management
│   │   │
│   │   ├── 📁 auth/                 ← Login callback handling
│   │   │
│   │   ├── 📁 login/                ← Teacher login page
│   │   │
│   │   ├── 📁 signup/               ← Teacher signup page
│   │   │
│   │   ├── 📁 quiz/                 ← Quiz taking pages
│   │   │   └── [token]/             ← Dynamic quiz page (one per link)
│   │   │
│   │   ├── 📁 results/              ← Results viewing
│   │   │   └── [attemptId]/         ← Dynamic results page
│   │   │
│   │   ├── 📁 teacher/              ← Teacher dashboard & tools
│   │   │   ├── page.tsx             ← Main dashboard
│   │   │   └── assignments/         ← Quiz management
│   │   │       ├── new/             ← Create new quiz
│   │   │       └── [id]/            ← Edit specific quiz
│   │   │           └── submissions/ ← View student submissions
│   │   │
│   │   └── middleware.ts            ← Route protection (login checks)
│   │
│   ├── 📁 components/               ← Reusable UI pieces
│   │   └── ui/                      ← Buttons, cards, forms, etc.
│   │
│   └── 📁 lib/                      ← Utilities and helpers
│       ├── 📁 grading/              ← Grading logic
│       │   ├── mcq.ts               ← MCQ grading
│       │   ├── open-answer.ts       ← AI grading
│       │   └── level-calculator.ts  ← Score → Level
│       ├── 📁 openai/               ← AI connection
│       │   └── client.ts            ← OpenAI API wrapper
│       └── 📁 supabase/             ← Database connection
│           ├── client.ts            ← Browser database access
│           ├── server.ts            ← Server database access
│           └── types.ts             ← Data type definitions
│
├── 📄 package.json                  ← Project dependencies
├── 📄 tsconfig.json                 ← TypeScript settings
├── 📄 .env.local                    ← Secret keys (not in git!)
└── 📄 README.md                     ← Project documentation
```

### Key Files Explained

| File | What It Does |
|------|--------------|
| `src/app/api/quiz/submit/route.ts` | The main "brain" - handles quiz submission, grading, and leveling |
| `src/lib/grading/open-answer.ts` | Sends answers to AI and gets scores back |
| `src/lib/grading/slider.ts` | Grades slider questions deterministically |
| `src/lib/grading/image-map.ts` | Grades image-map flags (deterministic + AI text flags) |
| `src/lib/grading/level-calculator.ts` | Converts percentage score to Beginner/Intermediate/Advanced |
| `src/middleware.ts` | Guards teacher pages - redirects non-logged-in users |
| `src/lib/supabase/types.ts` | Defines the shape of all data in the database |

---

**Previous:** [How It Works](./02-how-it-works.md)
**Next:** [Database](./04-database.md)
